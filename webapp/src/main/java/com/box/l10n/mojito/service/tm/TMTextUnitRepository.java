...
public interface TMTextUnitRepository extends JpaRepository<TMTextUnit, Long> {
  ...
  @Query(
      "SELECT new com.box.l10n.mojito.service.tm.BranchTextUnitVariantDTO("
          ...
          + "LEFT JOIN TMTextUnitCurrentVariant tucv ON (tu.id = tucv.tmTextUnit.id AND rl.locale.id = tucv.locale.id) "
          + "LEFT JOIN TMTextUnitVariant tuv ON tucv.tmTextUnitVariant.id = tuv.id "
